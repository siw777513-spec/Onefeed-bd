...
export const CreatePostModal: React.FC<Props> = ({ isOpen, onClose, defaultColumn='feed', currentUser }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let finalUrl = "https://images.unsplash.com/photo-1518709268805-4e9042af9f23?auto=format&fit=crop&w=800&q=80";
    ...
    if (file) {
      ...
      try {
        const res = await fetch(`https://api.cloudinary.com/v1_1/${CLOUD_NAME}/${mediaType==='video'?'video':'image'}/upload`, { method: 'POST', body: form });
    ...
    await addDoc(collection(db, "posts"), {
      text: text.trim(), image: finalUrl, videoUrl: mediaType==='video'?finalUrl:null,
      mediaType, column, userId: auth.currentUser?.uid,
      userName: currentUser?.name || auth.currentUser?.displayName || 'User',
      userAvatar: currentUser?.avatar, likes: [], timestamp: serverTimestamp()
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-black/85">
      <div className="w-full max-w-lg bg-[#12121A] rounded-3xl border border-white/10">
        <div className="flex justify-between p-4 border-b border-white/10"><h3 className="text-white font-bold">Create Post</h3><button onClick={onClose}><X className="text-white w-5 h-5"/></button></div>
        <form onSubmit={handleSubmit} className="p-4 space-y-3">
          ...
          <textarea value={text} onChange={e=>setText(e.target.value)} placeholder="Caption..." rows={3} required className="w-full bg-black border border-white/10 rounded-2xl p-3 text-xs text-white"/>

...
export const LoginModal: React.FC<LoginModalProps> = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newUser: UserProfile = {
        ...
        email: cleanEmail,
        avatar: isOwner? 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=200&q=80' : `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(cleanName)}`,
  ...
  return (
    // তোমার আগের Design টাই থাকবে, শুধু handleLogin Change হলো
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0A0A10]/95 backdrop-blur-xl">
      <div className="relative w-full max-w-md bg-[#12121E] border border-white/10 rounded-3xl p-8 shadow-2xl text-slate-100 flex flex-col items-center">
        ...
        <form onSubmit={handleLogin} className="w-full space-y-4">
          <div><label className="block text-xs font-bold text-slate-300 mb-1.5">Full Name</label><div className="relative"><User className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" /><input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your full name" className="w-full bg-[#0A0A10] border border-white/15 rounded-xl pl-9 pr-3 py-3 text-sm text-white" required /></div></div>

...
useEffect(()=>{(async()=>{if(!cur) return;
const d=await getDoc(doc(db,'users',cur.uid));
if(d.exists()) setPriv(d.data().isPrivate||false);})();},[cur]);
...
await updateDoc(doc(db,'posts',p.id),{likes:l?arrayRemove(cur!.uid):
arrayUnion(cur!.uid),likeCount:l?(p.likeCount||0)-1:(p.likeCount||0)+1});};
...
<div className="bg-white/10 backdrop-blur-[25px] border border-white/20 rounded-[22px] px-4 py-3 flex justify-between">
...
<div key={p.id} className="rounded-[20px] bg-white/10 border border-white/20 overflow-hidden">
...
<button onClick={()=>setOpenC(openC===p.id?null:p.id)} className="px-3 py-1.5 rounded-full bg-white/10 text-white/50 text-[11px]">Cmnt</button>
...
<button onClick={()=>comment(p)} className="px-3 py-1.5 rounded-full bg-white text-black text-[10px]">Send</button>
</div>}
</div>
</div>)})}
</div>
...
<div key={p.id} onClick={()=>openProfile(p.userId,p.userName)} className="bg-white/10 border border-white/15 rounded-2xl p-3 flex gap-3 items-center">
...
<div className="flex-1" onClick={()=>openChat(uid,ud.userName||'User')}>
...
<button className="w-11 h-6 rounded-full bg-white/20 flex p-1">
<div className="w-4 h-4 bg-white rounded-full"/>
</button>
</div>
<div className="flex justify-between items-center">
<p className="text-white text-xs">Show Online Status</p>
<button className="w-11 h-6 rounded-full bg-cyan-400 flex p-1 justify-end">
<div className="w-4 h-4 bg-white rounded-full"/>
</button>
</div>
</div>
...
<div className="bg-white/10 border border-white/15 rounded-[20px] p-4">
<p className="text-white/40 text-[10px] mb-2">NOTIFICATIONS</p>
<div className="space-y-3">
<div className="flex justify-between items-center">
...
<div className="bg-white/10 border border-white/15 rounded-[20px] p-4 space-y-2">
<p className="text-white/40 text-[10px]">APP SETTINGS</p>
...
<div className="bg-white/10 border border-white/15 rounded-[20px] p-4 space-y-2">
<p className="text-white/40 text-[10px]">STORAGE</p>
...
<div className="bg-white/10 border border-white/15 rounded-[20px] p-4 space-y-2">
<p className="text-white/40 text-[10px]">SUPPORT</p>
...
<div className="bg-white/10 border border-white/15 rounded-[20px] p-4 space-y-3">
...
{bot==='Profile'&&<div className="flex-1 pb-24">
...
<label className="block mt-2 py-2 rounded-xl bg-white/5 border border-dashed border-white/10 text-center text-xs text-white/40 cursor-pointer">Media

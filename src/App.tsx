...
function FeedApp({onLogout}:{onLogout:()=>void}){
 ...
 useEffect(()=>{
  return onSnapshot(query(collection(db,'posts'),orderBy('timestamp','desc')),s=>setPosts(s.docs.map(d=>({id:d.id,...d.data()} as Post))))
 },[]);
 useEffect(()=>{
  return onSnapshot(collection(db,'users'),s=>{const m:any={}; s.docs.forEach(d=>m[d.id]=d.data()); setUMap(m);})
 },[]);
 useEffect(()=>{(async()=>{if(!cur) return; const d=await getDoc(doc(db,'users',cur.uid)); if(d.exists()) setPriv(d.data().isPrivate||false);})();},[cur]);
 useEffect(()=>{
  if(!chatId) return;
  return onSnapshot(query(collection(db,'chats',chatId,'messages'),orderBy('time','asc')),s=>setMsgs(s.docs.map(d=>({id:d.id,...d.data()} as Msg))));
 },[chatId]);
 const togglePriv=async()=>{
  const v=!priv; setPriv(v);
  if(cur) await setDoc(doc(db,'users',cur.uid),{isPrivate:v,userName:cur?.email?.split('@')[0],userId:cur.uid},{merge:true});
 };
 const like=async(p:Post)=>{
  const l=p.likes?.includes(cur!.uid);
  await updateDoc(doc(db,'posts',p.id),{likes:l?arrayRemove(cur!.uid):arrayUnion(cur!.uid),likeCount:l?(p.likeCount||0)-1:(p.likeCount||0)+1});
 };
 const comment=async(p:Post)=>{
  if(!cmt.trim()) return;
  await updateDoc(doc(db,'posts',p.id),{comments:arrayUnion({id:Date.now().toString(),user:cur?.email?.split('@')[0],text:cmt})});
  setCmt('');
 };
 ...
 const create=async()=>{
  if(!txt && !media) return;
  await addDoc(collection(db,'posts'),{text:txt,image:ct==='feed'||ct==='story'?media:'',videoUrl:ct==='short'||ct==='watch'?media:'',userName:cur?.email?.split('@')[0],userId:cur?.uid,likeCount:0,likes:[],comments:[],mediaType:ct,timestamp:serverTimestamp()});
 ...
 const sendMsg=async()=>{
  ...
  await addDoc(collection(db,'chats',chatId,'messages'),{text:msgTxt,from:cur.uid,to:chatUser.id,time:serverTimestamp()});
  setMsgTxt('');
 };
 return(
  ...
  <div className="relative z-10 h-full flex flex-col p-3">
   <div className="bg-white/10 backdrop-blur-[25px] border border-white/20 rounded-[22px] px-4 py-3 flex justify-between items-center">
    <div className="flex gap-2 items-center">
     ...
     <p className="text-white font-black text-[12px]">OneFeedBD {priv?'Private':'Public'}</p>
    </div>
   ...
   <div className="flex-1 mt-3 overflow-hidden flex flex-col">
    {bot==='Home'&&<>
     <div className="flex gap-1 p-1 rounded-full bg-white/10 border border-white/10">
      {['Feed','Short','Watch','Story'].map(k=>
       <button key={k} onClick={()=>setTab(k)} className={`flex-1 py-2 rounded-full text-[10px] font-black ${tab===k?'bg-white text-black':'text-white/40'}`}>{k}</button>
      )}
     </div>
      ...
      {filt.map(p=>{
       ...
       <div key={p.id} className="rounded-[20px] bg-white/10 backdrop-blur-xl border border-white/20 overflow-hidden">
        <div className="flex justify-between p-3">
         ...
         <button onClick={()=>openChat(p.userId,p.userName)} className="w-7 h-7 rounded-full bg-cyan-500 text-white text-[10px]">M</button>
        </div>
        ...
        <div className="p-3">
         ...
         <div className="flex gap-2 mt-3">
          <button onClick={()=>like(p)} className={`px-3 py-1.5 rounded-full text-[11px] ${liked?'bg-pink-500/20 text-pink-200':'bg-white/10 text-white/50'}`}>{(p.likeCount||0)} Like</button>
          <button onClick={()=>{setOpenC(openC===p.id?null:p.id)}} className="px-3 py-1.5 rounded-full bg-white/10 text-white/50 text-[11px]">Comment</button>
          {cur?.uid===p.userId&&<button onClick={()=>del(p)} className="px-3 py-1.5 rounded-full bg-red-500/10 text-red-300 text-[11px]">Del</button>}
         </div>
     ...
     <div className="space-y-2 overflow-y-auto">
      {Object.entries(uMap).filter(([uid])=>uid!==cur?.uid).map(([uid,udata]:any)=>
       <div key={uid} className="bg-white/10 border border-white/15 rounded-2xl p-3 flex gap-3 items-center">
        ...
        <button onClick={()=>openChat(uid,udata.userName||uid.slice(0,6))} className="w-8 h-8 rounded-full bg-white text-black font-bold">M</button>
       </div>
      )}
     </div>
    </div>}
    {bot==='Chat'&&chatUser&&<div className="flex-1 flex flex-col pb-3">
     ...
     <div className="mt-3 flex gap-2">
      ...
      <button onClick={sendMsg} className="w-10 h-10 rounded-full bg-white text-black font-black">S</button>
     </div>
    </div>}
     ...
     <div className="space-y-3">
      <div className="bg-white/10 border border-white/15 rounded-[20px] p-4">
       ...
       <div className="flex justify-between items-center">
        <p className="text-white text-xs">{priv?'Private':'Public'} Account</p>
      ...
      <div className="bg-white/10 border border-white/15 rounded-[20px] p-4">
       ...
       <button className="w-full flex justify-between items-center"><p className="text-white text-xs">Edit Profile</p><p className="text-white/20">></p></button>
       <button className="w-full flex justify-between items-center mt-3"><p className="text-white text-xs">Blocked Users</p><p className="text-white/20">0 ></p></button>
      </div>
      <div className="bg-white/10 border border-white/15 rounded-[20px] p-4">
       <p className="text-white/40 text-[10px] font-black mb-3">NOTIFICATIONS</p>
       <div className="flex justify-between items-center"><p className="text-white text-xs">Like Notifications</p><button className="w-12 h-6 rounded-full bg-cyan-400 flex items-center p-1 justify-end"><div className="w-4 h-4 rounded-full bg-white"/></button></div>
       <div className="flex justify-between items-center mt-3"><p className="text-white text-xs">Comment Notifications</p><button className="w-12 h-6 rounded-full bg-cyan-400 flex items-center p-1 justify-end"><div className="w-4 h-4 rounded-full bg-white"/></button></div>
      </div>
      <div className="bg-white/10 border border-white/15 rounded-[20px] p-4">
       <p className="text-white/40 text-[10px] font-black mb-3">APP</p>
       <button className="w-full flex justify-between items-center"><p className="text-white text-xs">App Version</p><p className="text-white/40 text-[10px]">v2.0.0</p></button>
       <button className="w-full flex justify-between items-center mt-3"><p className="text-white text-xs">Clear Cache</p><p className="text-white/40 text-[10px]">12.5 MB</p></button>
      </div>
      ...
      <button className="w-full py-3 rounded-full bg-red-500/20 border border-red-500/30 text-red-400 text-xs font-black">DELETE ACCOUNT</button>
     </div>
    </div>}
    <div className="absolute bottom-3 left-3 right-3 bg-white/10 backdrop-blur-[30px] border border-white/20 rounded-[28px] flex justify-around items-center py-2">
     {[{k:'Home',i:'H'},{k:'Search',i:'S'},{k:'Add',i:'+'},{k:'Inbox',i:'M'},{k:'Profile',i:'P'}].map(b=> b.k==='Add'? <button key={b.k} onClick={()=>setShow(true)} className="w-10 h-10 rounded-full bg-white text-black font-black">+</button>:<button key={b.k} onClick={()=>setBot(b.k)} className={`w-8 h-8 rounded-full ${bot===b.k?'bg-white/15 text-white':'text-white/30'}`}>{b.i}</button>)}
    </div>
   </div>
   {show&&<div className="absolute inset-0 z-50 bg-black/50 backdrop-blur-xl flex items-center justify-center p-4">
    <div className="w-full bg-white/15 border border-white/20 rounded-[20px] p-4">
     <div className="flex justify-between mb-3"><h3 className="text-white font-bold">Create</h3><button onClick={()=>setShow(false)} className="w-7 h-7 rounded-full bg-white/10 text-white/50">X</button></div>
...
export default function App(){
 ...
 useEffect(()=>{
  return onAuthStateChanged(auth,u=>{if(u) setMode('auth'); else if(mode!=='guest') setMode('none'); setChk(false);});
 },[]);
 if(chk) return <div className="min-

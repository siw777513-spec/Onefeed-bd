          ...
          <div className="flex-1 mt-3 overflow-hidden flex flex-col">
            {activeBottom==='Home' && (<><div className="flex gap-1 p-1 rounded-full bg-white/10 border border-white/10">{['Feed','Short','Watch','Story'].map(k=><button key={k} onClick={()=>setTab(k)} className={`flex-1 py-2.5 rounded-full text-[10px] font-black ${tab===k?'bg-white text-black':'text-white/40'}`}>{k}</button>)}</div>
              <div className="flex-1 overflow-y-auto pb-28 mt-3 space-y-3">
                {filtered.map(p=>{ const liked=p.likes?.includes(currentUser?.uid||''); const canDelete=p.userId===currentUser?.uid||isAdmin; return (<div key={p.id} className="rounded-[24px] bg-white/10 backdrop-blur-[25px] border border-white/20 overflow-hidden"><div className="flex justify-between p-3"><div className="flex gap-2.5 items-center"><img src={`https://i.pravatar.cc/100?u=${p.userId}`} className="w-8 h-8 rounded-full"/><div><p className="text-white text-xs font-bold">{p.userName}</p><p className="text-white/40 text-[9px]">{p.mediaType}</p></div></div><button onClick={()=>setOpenMenuId(openMenuId===p.id?null:p.id)} className="w-8 h-8 rounded-full bg-white/10 text-white/40">⋯</button></div>{p.image&&<img src={p.image} className="w-full"/>}{p.videoUrl&&<video src={p.videoUrl} controls className="w-full"/>}<div className="p-3"><p className="text-white/90 text-[13px]">{p.text}</p><div className="flex gap-2 mt-3 pt-3 border-t border-white/10"><button onClick={()=>handleLike(p)} className={`px-4 py-2 rounded-full border text-[11px] font-bold ${liked?'bg-pink-500/20 border-pink-300/30 text-pink-200':'bg-white/10 border-white/10 text-white/50'}`}>{liked?'❤️':'🤍'} {p.likeCount}</button><button onClick={()=>setOpenCommentId(openCommentId===p.id?null:p.id)} className="px-4 py-2 rounded-full bg-white/10 border border-white/10 text-white/50 text-[11px] font-bold">💬 {p.comments?.length||0}</button><button onClick={()=>{navigator.clipboard.writeText(location.href); alert('Copy ✅');}} className="ml-auto px-4 py-2 rounded-full bg-white/10 border border-white/10 text-white/50 text-[11px]">↗ Share</button></div>{canDelete&&openMenuId===p.id&&<button onClick={()=>handleDelete(p)} className="mt-2 px-3 py-1.5 rounded-full bg-red-500/10 border border-red-500/20 text-red-300 text-[11px]">🗑️ Delete</button>}{openCommentId===p.id&&<div className="mt-3 bg-black/30 border border-white/10 rounded-xl p-3"><div className="space-y-1 mb-2 max-h-24 overflow-y-auto">{p.comments?.map((c:any)=><p key={c.id} className="text-[11px] text-white/60"><span className="text-cyan-200 font-bold">{c.user}: </span>{c.text}</p>)}</div><div className="flex gap-2"><input value={commentText} onChange={e=>setCommentText(e.target.value)} placeholder="Comment..." className="flex-1 bg-white/10 border border-white/10 rounded-full px-3 py-1.5 text-xs text-white outline-none"/><button onClick={()=>handleComment(p)} className="px-3 rounded-full bg-white text-black text-xs font-bold">Send</button></div></div>}</div></div>);})}</div></>)}

            {activeBottom==='Search' && <div className="flex-1 pb-24"><div className="bg-white/10 backdrop-blur-xl border border-white/15 rounded-full px-5 py-3 flex gap-2"><span className="text-white/30">⌕</span><input value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder="Search..." className="flex-1 bg-transparent text-sm text-white outline-none"/></div><div className="mt-4 space-y-2">{(searchQ?searched:posts).slice(0,10).map(p=><div key={p.id} className="bg-white/10 border border-white/15 rounded-2xl p-3 flex gap-3"><img src={p.image||`https://i.pravatar.cc/100?u=${p.userId}`} className="w-12 h-12 rounded-xl object-cover"/><div><p className="text-white text-xs font-bold">{p.userName}</p><p className="text-white/40 text-[11px] truncate w-[200px]">{p.text}</p></div></div>)}</div></div>}

            {activeBottom==='Inbox' && <div className="flex-1 pb-24"><h2 className="text-white font-black text-xl">Inbox 💧</h2><p className="text-white/40 text-[11px] mb-4">5 online</p><div className="space-y-2">{posts.slice(0,8).map((p,i)=><div key={p.id} className="bg-white/10 backdrop-blur-xl border border-white/15 rounded-[18px] p-3.5 flex gap-3 items-center"><img src={`https://i.pravatar.cc/100?u=${p.userId}`} className="w-11 h-11 rounded-full"/><div className="flex-1"><p className="text-white text-[13px] font-bold">{p.userName}</p><p className="text-white/40 text-[11px] truncate">Water glass message...</p></div><div className="text-right"><p className="text-white/20 text-[9px]">2m</p>{i<2&&<div className="w-5 h-5 rounded-full bg-cyan-400 text-black flex items-center justify-center text-[9px] font-black mt-1">2</div>}</div></div>)}</div></div>}

            {activeBottom==='Profile' && <div className="flex-1 overflow-y-auto pb-28"><div className="bg-white/10 backdrop-blur-[30px] border border-white/20 rounded-[28px] p-6 text-center"><img src={`https://i.pravatar.cc/150?u=${currentUser?.uid}`} className="w-24 h-24 rounded-full mx-auto border-2 border-white/30"/><h3 className="text-white font-black mt-3">{currentUser?.email?.split('@')[0]}</h3><p className="text-white/40 text-[11px]">{currentUser?.email}</p><div className="grid grid-cols-3 gap-3 mt-5"><div className="bg-black/20 border border-white/15 rounded-2xl py-4"><p className="text-white font-black text-[20px]">{myPosts.length}</p><p className="text-white/60 text-[9px] font-bold">POSTS</p></div><div className="bg-black/20 border border-white/15 rounded-2xl py-4"><p className="text-pink-300 font-black text-[20px]">{totalLikes}</p><p className="text-white/60 text-[9px] font-bold">LIKES ❤️</p></div><div className="bg-black/20 border border-white/15 rounded-2xl py-4"><p className="text-cyan-300 font-black text-[20px]">1.2K</p><p className="text-white/60 text-[9px] font-bold">FOLLOWERS</p></div></div><div className="flex gap-2 mt-5"><button onClick={()=>setActiveBottom('Settings')} className="flex-1 py-3 rounded-full bg-white text-black font-black text-[11px]">EDIT PROFILE</button><button onClick={()=>setActiveBottom('Settings')} className="flex-1 py-3 rounded-full bg-white/10 border border-white/20 text-white text-[11px]">⚙️ SETTINGS</button></div><button onClick={()=>setActiveBottom('Inbox')} className="w-full mt-3 py-3 rounded-full bg-cyan-400/15 border border-cyan-400/20 text-cyan-200 text-[11px] font-black">💬 MESSAGE</button></div></div>}

            {activeBottom==='Settings' && <div className="flex-1 pb-28"><div className="flex items-center gap-3 mb-5"><button onClick={()=>setActiveBottom('Profile')} className="w-9 h-9 rounded-full bg-white/10 border border-white/10 text-white">‹</button><h2 className="text-white font-black text-lg">Settings</h2></div><div className="space-y-3"><div className="bg-white/10 border border-white/15 rounded-2xl p-4"><p className="text-white/30 text-[9px] mb-3">ACCOUNT</p><div className="space-y-3"><div className="flex justify-between"><p className="text-white text-xs">👤 Edit Profile</p><span className="text-white/20">›</span></div><div className="flex justify-between"><p className="text-white text-xs">🔒 Privacy</p><span className="text-white/20">›</span></div><div className="flex justify-between"><p className="text-white text-xs">🔔 Notifications</p><span className="text-white/20">›</span></div></div></div><div className="bg-white/10 border border-white/15 rounded-2xl p-4"><p className="text-white/30 text-[9px] mb-3">APP</p><div className="flex gap-3 items-center"><div className="w-8 h-8 rounded-full bg-cyan-400/20 flex items-center justify-center">💧</div><div><p className="text-white text-xs font-bold">Water Glass Theme</p><p className="text-cyan-200/50 text-[10px]">ON</p></div></div></div><button onClick={onLogout} className="w-full py-4 rounded-full bg-red-500/10 border border-red-500/20 text-red-300 font-black text-xs">LOGOUT</button></div></div>}
...
export default function App() {
  ...
  if(checking) return <div className="min-h-screen bg-black flex items-center justify-center text-white/20">💧 Loading...</div>;
